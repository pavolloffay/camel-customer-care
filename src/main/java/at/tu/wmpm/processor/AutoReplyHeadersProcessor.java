...
public class AutoReplyHeadersProcessor {
    ...
    private static final Logger log = LoggerFactory
            .getLogger(AutoReplyHeadersProcessor.class);

    public void process(Exchange exchange) throws Exception {
        log.debug(ReflectionToStringBuilder.toString(exchange));

        Message inMessage = exchange.getIn();
        MailBusinessCase mailBusinessCase = inMessage
                .getBody(MailBusinessCase.class);
        ...
        outMessage.setHeader("CamelVelocityContext",
                getVelocityContext(mailBusinessCase.getComments().get(0)
                        .getMessage()));
        exchange.setOut(outMessage);
    }

    private Map<String, Object> prepareOutHeaders(
            MailBusinessCase mailBusinessCase) {
        ...
        outHeaders.put("Subject", "We have received your request, ID:"
                + mailBusinessCase.getId());
    ...
    private VelocityContext getVelocityContext(String oldBody) {
        ...
        oldBody = oldBody.replace(System.lineSeparator(),
